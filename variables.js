...
function updateVariablesForCue(self, cueData) {
  const valuesToSet = {};
  let cueIndex = -1;
  ...
  if (cueIndex !== -1 && cueIndex < MAX_CUES_FOR_VARIABLES) {
    ...
    valuesToSet[`${prefix}_playlist_item_name`] =
      cueData.playlistItemName || "";
    valuesToSet[`${prefix}_time_formatted`] = cueData.currentTimeFormatted;
    valuesToSet[`${prefix}_duration_formatted`] =
      cueData.totalDurationFormatted;
    valuesToSet[`${prefix}_remaining_formatted`] =
      cueData.remainingTimeFormatted;
    ...
    self.log(
      "debug",
      `Cue ${cueData.cueId} (${cueData.cueName}) is beyond variable range (${MAX_CUES_FOR_VARIABLES}). Not setting individual variables.`,
    );
  }
  ...
  if (cueData.status === "playing" || cueData.status === "paused") {
    ...
    valuesToSet[`${currentlyPlayingPrefix}_playlist_item_name`] =
      cueData.playlistItemName || "";
    valuesToSet[`${currentlyPlayingPrefix}_time_formatted`] =
      cueData.currentTimeFormatted;
    valuesToSet[`${currentlyPlayingPrefix}_duration_formatted`] =
      cueData.totalDurationFormatted;
    valuesToSet[`${currentlyPlayingPrefix}_remaining_formatted`] =
      cueData.remainingTimeFormatted;
    valuesToSet[`${currentlyPlayingPrefix}_time_sec`] = cueData.currentTimeSec;
    valuesToSet[`${currentlyPlayingPrefix}_duration_sec`] =
      cueData.totalDurationSec;
    valuesToSet[`${currentlyPlayingPrefix}_remaining_sec`] =
      cueData.remainingTimeSec;
  } else if (
    cueData.status === "stopped" &&
    self.currentlyPlayingCueId === cueData.cueId
  ) {
...
module.exports = { getVariableDefinitions, updateVariablesForCue };
